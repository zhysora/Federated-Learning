...
class GlobalModel(object): # 全局模型
    ...
    def update_weights_one(self, client_weights, p=.1): # 更新权重  仅使用一个client update
        new_weights = [np.zeros(w.shape) for w in self.current_weights] # 新权重 先 全部清零
        
        for i in range(len(new_weights)): # 枚举 某一个权重
                new_weights[i] += self.current_weights[i] * (1 - p) + client_weights[i] * p # 关于数据量的 加权平均
        self.current_weights = new_weights  # 更新权重
    ...
    def aggregate_test_loss_accuracy(self, loss, accuracy, cur_round):
        cur_time = int(round(time.time())) - self.training_start_time
        self.test_losses += [[cur_round, cur_time, loss]]
        self.test_accuracies += [[cur_round, cur_time, accuracy]]
        with open('stats.txt', 'w') as outfile:
            json.dump(self.get_stats(), outfile)
...
class FLServer(object):  # 服务端
    ...
    def register_handles(self):# socket.io 服务 相应 各个socketIO事件(event)
        ...
        def handle_client_update(data):
            ...
            if FLServer.SERVER_MODE:
                self.global_model.update_weights_one(pickle_string_to_obj(data['weights']), 1. / FLServer.MIN_NUM_WORKERS)
                self.current_round = self.current_round + 1

                print("### start a new round:", self.current_round)
                if self.current_round % FLServer.MIN_NUM_WORKERS == 0:
        ...
        def handle_client_eval(data): 
            ...
            if len(self.eval_client_updates) > FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND * .7:
                ...
                print("total_round:", self.current_round + 1)
                print("== done ==")
...
if __name__ == '__main__':
    ...
    server = FLServer(GlobalModel_MNIST_CNN, server_host, server_port, datasource.Mnist) # 服务端
